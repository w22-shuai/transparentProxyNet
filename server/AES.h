...
public:
    ...
    std::shared_ptr<std::vector<uint8_t>> doEncrypt(void *dataPtr, long long dataSize);
    std::shared_ptr<std::vector<uint8_t>> doDecrypt(void *dataPtr, long long dataSize);
};